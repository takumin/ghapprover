 ...
 * in src/client.ts; the frame each call is made in lives in src/contract-call.ts, and the mapping of
 * a thrown failure onto GithubApiError in src/api-error.ts.
 */
...
async function listPullRequestItems<Schema extends GenericSchema>(
	...
	const items = await contractItems(
		endpoint,
		async () => {
